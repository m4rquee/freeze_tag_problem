...
TOTAL_TIME = MAX_TIME
space, factor = read_tsplib_2d_graph(EPS)
...
d_names, d_coords, d_degrees, grid_map = discretize(space.coords, EPS)
d_n = len(d_names)
...
sol_edges = []
_, MAX_TIME = solve_ftp_inner(sol_edges, d_tree, source, space, grid_map, MAX_TIME)
...
biggest_cell = max_cluster_radius(space, d_names, grid_map)
...
plot_solution('dot', space, solution, upper_solution, EPS)
