...
bool solve(Problem_Instance &P, double &LB, double &UB, int max_degree = 3) {
    ...
    Digraph::ArcMap<GRBVar> x_e(P.g); // number of paths that passes through e
    Digraph::ArcMap<GRBVar> y_e(P.g); // if arc e is present in the tree
    Digraph::ArcMap<GRBVar> v_ek(P.g);// whether edge e is on the shortest path from the root to node k
    ...
    // Callback setup: -----------------------------------------------------------
#ifndef BDHST
    LocalSearchCB cb(P, x_e, y_e, v_ek, height);
    ...
    for (OutArcIt e(P.g, P.source); e != INVALID; ++e) s_out_degree_expr += y_e[e];
    model.addConstr(s_out_degree_expr == 1);
    GRBLinExpr s_in_degree_expr;
    for (InArcIt e(P.g, P.source); e != INVALID; ++e) s_in_degree_expr += y_e[e];
    ...
    for (DNodeIt v(P.g); v != INVALID; ++v) {
        ...
        for (InArcIt e(P.g, v); e != INVALID; ++e) in_degree_expr += y_e[e];
#ifdef BDHST
    ...
    for (ArcIt e(P.g); e != INVALID; ++e) arc_sum_expr += y_e[e];
        ...
        if (v != P.source)
            for (InArcIt e(P.g, v); e != INVALID; ++e) {
                ...
                if (UB <= P.weight[e]) {// cannot use this edge
                    model.addConstr(y_e[e] == 0);
                    continue;
                }
                model.addConstr(h_v[v] >= h_v[P.g.source(e)] + P.weight[e] + (P.weight[e] + UB) * (y_e[e] - 1));
                if (P.nnodes > 500) continue;// reduce the model size for big instances
                constrCount++;
                model.addConstr(h_v[v] <= h_v[P.g.source(e)] + P.weight[e] + (P.weight[e] + UB) * (1 - y_e[e]));
            }
