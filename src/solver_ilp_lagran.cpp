...
bool solve(Problem_Instance &P, double &LB, double &UB) {
    ...
    cout << "-> the source is the root - " << 1 << " constrs" << endl;

    // The in-degree is one for each internal node:
    constrCount = 0;
    for (DNodeIt v(P.g); v != INVALID; ++v) {
    ...
    cout << "-> the in-degree is one for each internal node - " << constrCount << " constrs" << endl;
    ...
    if (abs(Z_LB_k - Z_UB) > min_delta) {
        ...
        cout << "-> the source has out-degree one - " << 1 << " constrs" << endl;

        // The out-degree is at most two for each internal node:
        constrCount = 0;
        for (DNodeIt v(P.g); v != INVALID; ++v) {
            if (v == P.source) continue;
            GRBLinExpr out_degree_expr;
        ...
        cout << "-> the out-degree is at most two for each internal node - " << constrCount << " constrs\n\n";
